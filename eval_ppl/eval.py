...
def parse_config():
    ...
    parser.add_argument("--attn_type", type=str, default="naive", help="", choices=["naive", "rerope"])
    parser.add_argument("--data_path", type=str, default="./test.bin", help="")
    parser.add_argument("--rerope_window", type=int, default=1024, help="")
...
def evaluate(model, data, batch_size, device, seq_length, sliding_window=256, use_cache=False):
    ...
    with torch.no_grad():
        ...
        for idx, (x, y) in tqdm(
            enumerate(
                get_as_batch(data["val"], seq_length, batch_size, device=device, sliding_window=sliding_window)
            ),
            ...
            for part_idx, i in enumerate(range(0, x.shape[1], seq_length)):
                ...
                if args.attn_type == "rerope":
                    from ops.attn_rerope import ReRoPECache

                    extra_args = {"past_key_values": ReRoPECache(args.rerope_window)}
                else:
                    extra_args = {}
...
def main(args):
    ...
    match args.attn_type:
        case "rerope":
            from ops.attn_rerope import use_rerope
